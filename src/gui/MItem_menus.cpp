...
MI_CURRENT_PROFILE::MI_CURRENT_PROFILE()
    : WI_LABEL_t(_(label), IDR_NULL, is_enabled_t::yes, config_store().steel_sheets.get().num_of_calibrated() > 1 ? is_hidden_t::no : is_hidden_t::yes) {
...
MI_USB_MSC_ENABLE::MI_USB_MSC_ENABLE()
    : WI_SWITCH_OFF_ON_t(config_store().usb_msc_enabled.get(), _(label), IDR_NULL, is_enabled_t::yes, is_hidden_t::dev) {}
